...
class PTW_Plotter:
    ...
    def ptw_prediction_plots_hier(self, path, sel):
        """PTW Prediction Hierarchical Plots"""
        pred_theta_raw = [
        ...
        for i in range(self.setup.nexp):
            for j in range(self.setup.ntheta[i]):
                pred_theta.append(
                    pred_theta_raw[i].T[self.setup.s2_ind[i] == j].T
                )
                pred_theta_quant_lb.append(
                    np.quantile(pred_theta[-1], 0.005, 0)
                )
                pred_theta_quant_ub.append(
                    np.quantile(pred_theta[-1], 0.995, 0)
                )
                pred_theta0.append(
                    pred_theta0_raw[i].T[self.setup.s2_ind[i] == j].T
                )
                pred_theta0_quant_lb.append(
                    np.quantile(pred_theta0[-1], 0.005, 0)
                )
                pred_theta0_quant_ub.append(
                    np.quantile(pred_theta0[-1], 0.995, 0)
                )
                pred_thetap.append(
                    pred_thetap_raw[i].T[self.setup.s2_ind[i] == j].T
                )
                pred_thetap_quant_lb.append(
                    np.quantile(pred_thetap[-1], 0.005, 0)
                )
                pred_thetap_quant_ub.append(
                    np.quantile(pred_thetap[-1], 0.995, 0)
                )
                ...
                if model_type[-1] == "ModelMaterialStrength":
                    ...
                    real_strain.append(
                        self.setup.models[i].meas_strain_histories[j]
                    )
                    real_stress.append(
                        self.setup.ys[i][self.setup.s2_ind[i] == j]
                    )
                else:
                    edots.append(None)
                    temps.append(None)
                    real_strain.append(
                        np.linspace(
                            0,
                            1,
                            len(self.setup.ys[i][self.setup.s2_ind[i] == j]),
                        )
                    )
                    real_stress.append(
                        self.setup.ys[i][self.setup.s2_ind[i] == j]
    ...
    def ptw_prediction_plots_pool(self, path, sel):
        ...
        for i in range(self.setup.nexp):
            for j in range(self.setup.ns2[i]):
                ...
                pred_theta_quant_lb.append(
                    np.quantile(pred_theta[-1], 0.025, 0)
                )
                pred_theta_quant_ub.append(
                    np.quantile(pred_theta[-1], 0.975, 0)
                )
                model_type.append(type(self.setup.models[i]).__name__)
                if model_type[-1] == "ModelMaterialStrength":
                    edots.append(self.setup.models[i].edots[j])
    ...
    def ptw_prediction_plots_cluster(self, path, sel):
        ...
        for it, s in enumerate(sel):
            for i in range(self.setup.nexp):
                dcounts[it] += np.bincount(
                    self.out.delta[i][s, 0], minlength=nclustmax
                )
        ...
        for i in range(self.setup.nexp):
            for j in range(self.setup.ntheta[i]):
                ...
                pred_theta0_quant_lb.append(
                    np.quantile(pred_theta0[-1], 0.025, 0)
                )
                pred_theta0_quant_ub.append(
                    np.quantile(pred_theta0[-1], 0.975, 0)
                )
                pred_thetap.append(
                    pred_thetap_raw[i].T[self.setup.s2_ind[i] == j].T
                )
                pred_thetap_quant_lb.append(
                    np.quantile(pred_thetap[-1], 0.025, 0)
                )
                pred_thetap_quant_ub.append(
                    np.quantile(pred_thetap[-1], 0.975, 0)
                )
                edots.append(self.setup.models[i].edots[j])
                temps.append(self.setup.models[i].temps[j])
                real_strain.append(
        ...
        for i in range(self.setup.p):
            for j in range(self.setup.p):
                if i == j:
                    ...
                    plt.xlim(
                        self.setup.bounds_mat[i, 0], self.setup.bounds_mat[i, 1]
                    )
                    ...
                    contour = self.kde_contour(
                        theta0_unst[:, j], theta0_unst[:, i], 0.9
                    )
                    plt.contour(
                        contour["X"],
                        contour["Y"],
                    ...
                    contour = self.kde_contour(
                        theta_parent_unst[:, j], theta_parent_unst[:, i], 0.9
                    )
                    plt.contour(
                        contour["X"],
                        contour["Y"],
                    ...
                    plt.xlim(
                        self.setup.bounds_mat[j, 0], self.setup.bounds_mat[j, 1]
                    )
                    plt.ylim(
                        self.setup.bounds_mat[i, 0], self.setup.bounds_mat[i, 1]
                    )
    ...
    def cluster_matrix(delta_list, ns2, nclustmax, nburn=20000, nthin=10):
        ...
        bounds = [
            (breaks[i], breaks[i + 1]) for i in range(breaks.shape[0] - 1)
        ]
        delta_mat = np.empty(
    ...
    def cluster_matrix_plot(self, path=None, **kwargs):
        cmat, breaks = self.cluster_matrix(
            self.out.delta, self.setup.ns2, self.out.nclustmax, **kwargs
        )
        plt.matshow(cmat)
        if breaks.shape[0] > 1:
