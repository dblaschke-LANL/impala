...
def func_prediction_plot(
    ...
    actual_y,  # obs
    pred_y_lb,
    pred_y_ub,  # prediction (using thetai in hierarchical)
    theta0_y_lb=None,
    theta0_y_ub=None,  # hierarchical mean
    parent_y_lb=None,
    parent_y_ub=None,  # hierarchical parent distribution
    text=None,
    pdf=None,
    ylim=None,
    sigma=None,
    ...
    plt.fill_between(
        actual_x, pred_y_lb, pred_y_ub, color="lightgreen", label=r"$\theta_i$"
    )
    if sigma is not None:
        for i in range(len(actual_x)):
            plt.plot(
                [actual_x[i], actual_x[i]],
                [actual_y[i] - sigma, actual_y[i] + sigma],
                color="blue",
                linewidth=0.1,
            )
...
def ptw_prediction_plots_pool(
    ...
    for i in range(setup.nexp):
        for j in range(setup.ns2[i]):
            if type(setup.models[i]).__name__ == "ModelMaterialStrength":
                ...
                pred_theta_quant_lb.append(
                    np.percentile(pred_theta[-1], alpha / 2, 0)
                )
                pred_theta_quant_ub.append(
                    np.percentile(pred_theta[-1], 1 - alpha / 2, 0)
                )
                text.append(
                    "edot: "
                    + str(round(setup.models[i].edots[j] * 1e6))
                    + "/s\ntemp: "
                    + str(round(setup.models[i].temps[j]))
                    + "K"
                )
    ...
    if ylim == "constant":
        m1 = min(
            [pp.min() for pp in pred_theta_quant_lb]
            + [pp.min() for pp in obs_stress]
        )
        m2 = max(
            [pp.max() for pp in pred_theta_quant_ub]
            + [pp.max() for pp in obs_stress]
        )
        ylim = [m1, m2]

    pdf = PdfPages(path)
    for k in range(ntot):
        func_prediction_plot(
            actual_x=obs_strain[k],
            actual_y=obs_stress[k],
            pred_y_lb=pred_theta_quant_lb[k],
            pred_y_ub=pred_theta_quant_ub[k],
            text=text[k],
            pdf=pdf,
            ylim=ylim,
            sigma=sigma[k],
...
def ptw_prediction_plots_hier(
    ...
    for i in range(setup.nexp):
        ...
        pred_theta_raw[i] = setup.models[i].eval(
            sc.tran_unif(
                calib_out.theta[i][mcmc_use, 0, :, :].reshape([-1, setup.p]),
                setup.bounds_mat,
                setup.bounds.keys(),
            ),
            pool=False,
        ...
        pred_thetap_raw[i] = setup.models[i].eval(
            sc.tran_unif(theta_parent, setup.bounds_mat, setup.bounds.keys()),
            pool=True,
            nugget=True,
        )
    ...
    for i in range(setup.nexp):
        for j in range(setup.ntheta[i]):
            if type(setup.models[i]).__name__ == "ModelMaterialStrength":
                ...
                pred_theta0_quant_lb.append(
                    np.percentile(pred_theta0[-1], alpha / 2, 0)
                )
                pred_theta0_quant_ub.append(
                    np.percentile(pred_theta0[-1], 1 - alpha / 2, 0)
                )
                pred_thetap.append(pred_thetap_raw[i].T[setup.s2_ind[i] == j].T)
                pred_thetap_quant_lb.append(
                    np.percentile(pred_thetap[-1], alpha / 2, 0)
                )
                pred_thetap_quant_ub.append(
                    np.percentile(pred_thetap[-1], 1 - alpha / 2, 0)
                )
                text.append(
                    "edot: "
                    + str(round(setup.models[i].edots[j] * 1e6))
    ...
    if ylim == "constant":
        m1 = min(
            [pp.min() for pp in pred_thetap_quant_lb]
            + [pp.min() for pp in obs_stress]
        )
        m2 = max(
            [pp.max() for pp in pred_thetap_quant_ub]
            + [pp.max() for pp in obs_stress]
        )
        ylim = [m1, m2]
    ...
    for k in range(ntot):
        func_prediction_plot(
            ...
            ylim=ylim,
            sigma=sigma[k],
            theta0_y_lb=pred_theta0_quant_lb[k],
            theta0_y_ub=pred_theta0_quant_ub[k],  # hierarchical mean
            parent_y_lb=pred_thetap_quant_lb[k],
...
def ptw_prediction_plots_cluster(
    ...
    for it, s in enumerate(mcmc_use):
        for i in range(setup.nexp):
            dcounts[it] += np.bincount(
                calib_out.delta[i][s, 0], minlength=nclustmax
            )
    etas = calib_out.eta[mcmc_use, 0]
    nclust = (dcounts > 0).sum(axis=1)
    prob = dcounts + (dcounts == 0) * (
        etas / (nclustmax - nclust + 1e-9)
    ).reshape(-1, 1)
    ...
    for i in range(setup.nexp):
        for j in range(mcmc_use.shape[0]):
            ...
            pred_theta0_raw[i][j] = setup.models[i].eval(
                sc.tran_unif(
                    np.repeat(
                        calib_out.theta0[mcmc_use[j], 0].reshape(1, -1),
                        setup.ns2[i],
                        axis=0,
                    ),
                    setup.bounds_mat,
            ...
            pred_thetap_raw[i][j] = setup.models[i].eval(
                sc.tran_unif(
                    np.repeat(
                        theta_parent[j].reshape(1, -1), setup.ns2[i], axis=0
                    ),
                    setup.bounds_mat,
                    setup.bounds.keys(),
                ),
...
def pairwise_theta_plot_hier(
    ...
    theta0_unst = sc.unnormalize(
        calib_out.theta0[mcmc_use, 0, :], setup.bounds_mat
    )
    theta_parent_unst = sc.unnormalize(theta_parent, setup.bounds_mat)
    theta_unst = [
        calib_out.theta[k][mcmc_use, 0, :, :] for k in range(setup.nexp)
    ]
    for k in range(setup.nexp):
        for s in range(setup.ntheta[k]):
            theta_unst[k][:, s, :] = sc.unnormalize(
                calib_out.theta[k][mcmc_use, 0, s, :], setup.bounds_mat
            )
    ...
    for i in range(setup.p):
        for j in range(setup.p):
            if i == j:
                ...
                for k in range(setup.nexp):
                    ...
                    if highlight[k] is not None:
                        for s in highlight[k]:
                            sns.kdeplot(
                                theta_unst[k][:, s, i], color="lightgreen"
                            )
                    ...
                    if highlight[k] is not None:
                        for s in highlight[k]:
                            ...
                            plt.contour(
                                ...
                                contour["conts"],
                                colors="lightgreen",
                                linestyles=lty[cnt % len(lty)],
                            )
                contour = kde_contour(
                    theta0_unst[:, j], theta0_unst[:, i], 1 - alpha
                )
                plt.contour(
                    contour["X"],
                    contour["Y"],
                ...
                contour = kde_contour(
                    theta_parent_unst[:, j], theta_parent_unst[:, i], 1 - alpha
                )
                plt.contour(
                    contour["X"],
                    contour["Y"],
...
def pairwise_theta_plot_pool_compare(
    setup, calib_out_list, cols, path, mcmc_use, alpha=0.05
):
    """Pairwise Theta scatterplot"""
    theta_names = list(setup.bounds.keys())
    theta0_unst_list = [
        sc.unnormalize(x.theta[mcmc_use, 0, :], setup.bounds_mat)
        for x in calib_out_list
    ]
    ...
    for i in range(setup.p):
        for j in range(setup.p):
            if i == j:
                ...
                plt.subplot2grid((setup.p, setup.p), (i, j))
                contour_list = [
                    kde_contour(x[:, j], x[:, i], 1 - alpha)
                    for x in theta0_unst_list
                ]
                for k in range(n):
                    plt.contour(
                        contour_list[k]["X"],
...
def pairwise_theta_plot_cluster(
    ...
    theta_parent_unst = sc.unnormalize(theta_parent, setup.bounds_mat)
    theta_unst = [
        calib_out.theta_hist[k][mcmc_use, 0, :, :] for k in range(setup.nexp)
    ]
    for k in range(setup.nexp):
        for s in range(setup.ntheta[k]):
            theta_unst[k][:, s, :] = sc.unnormalize(
...
def cluster_matrix(delta_list, ns2, nclustmax, nburn=20000, nthin=10):
    ...
    delta_mat = np.empty(
        (delta_relist[0].shape[0], breaks[-1], nclustmax), dtype=bool
    )
    # Fill the combined delta array
    for i in range(breaks.shape[0] - 1):
        delta_mat[:, bounds[i][0] : bounds[i][1]] = delta_relist[i][
            :, 0, :, None
        ] == np.arange(nclustmax)
...
def cluster_matrix_plot(setup, calib_out, path=None, **kwargs):
    cmat, breaks = cluster_matrix(
        calib_out.delta, setup.ns2, calib_out.nclustmax, **kwargs
    )
    plt.matshow(cmat)
    if breaks.shape[0] > 1:
...
def pairs(setup, mat_st, col=None, s=None, path=None):
    ...
    g.map_upper(
        hide_current_axis
    )  # to make compatible with early version of seaborn
...
def total_temperature_swaps(out, setup):
    ...
    if "theta0" in dir(out):
        ...
        total_swaps = (
            out.count_temper.sum() / 2
        )  # symmetric matrix, number of accepted swaps
        swaps_per_iter = total_swaps / (
            out.theta0.shape[0] - setup.start_temper
        )
        attempts_per_iter = setup.nswap_per * setup.nswap
        ax.text(
            out.count_temper.shape[0] * 0.9,
            out.count_temper.shape[0] * 0.1,
            s="Prop. Swaps Accepted: "
            + str(round(swaps_per_iter / attempts_per_iter, 3)),
            color="white",
            horizontalalignment="right",
        )
    else:
        ...
        attempts_per_iter = setup.nswap_per * setup.nswap
        ax.text(
            out.count.shape[0] * 0.9,
            out.count.shape[0] * 0.1,
            s="Prop. Swaps Accepted: "
            + str(round(swaps_per_iter / attempts_per_iter, 3)),
            color="white",
            horizontalalignment="right",
...
def save_parent_strength(setup, ptw_mod, calib_out, mcmc_use, path):
    ...
    theta_parent_native_dict = sc.tran_unif(
        theta_parent, setup.bounds_mat, setup.bounds.keys()
    )
...
def get_bounds(edot, strain, temp, results_csv, write_path, percentiles=None):
    ...
    model_ptw_star = sc.ModelMaterialStrength(
        ...
        consts=consts,
        strain_histories=[np.arange(0, strain, 0.01)],
...
def get_outcome_predictions_impala(setup, theta_input, disc_input=None):
    ...
    CONSTRAINTS = setup.checkConstraints(
        sc.tran_unif(
            sc.normalize(theta_input, setup.bounds_mat),
            setup.bounds_mat,
            setup.bounds.keys(),
        ),
        setup.bounds,
    ...
    if str(type(disc_input)) == "<class 'NoneType'>":
        for i in range(setup.nexp):
            outcome_draws[i] = setup.models[i].eval(
                sc.tran_unif(
                    sc.normalize(theta_input, setup.bounds_mat),
                    setup.bounds_mat,
                    setup.bounds.keys(),
        ...
        v_cur_list = [
            disc_input[:, 0 : disc_dims[i]]
            if i == 0
            else disc_input[
                :,
                int(np.sum(disc_dims[0 : (i - 1)])) : int(
                    np.sum(disc_dims[0:i])
                ),
            ]
            for i in range(setup.nexp)
        ]
        for i in range(setup.nexp):
            disc_y = (
                (setup.models[i].D @ v_cur_list[i].T).T
                if setup.models[i].nd > 0
                else np.repeat(0, len(setup.ys[i]))
            )
            outcome_draws[i] = (
                setup.models[i].eval(
                    sc.tran_unif(
