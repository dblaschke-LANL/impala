...
def get_map_impalapool(
        ...
        if setup.models[i] == "fix":
            ...
            for j in range(len(setup.ig_a[i])):
                if setup.s2_df[i][j] == 0:
                    s2[i][:, j] = halfcauchy.rvs(
                        loc=0, scale=1, size=n_samples
                    ).reshape(1, n_samples)
                else:
                    s2[i][:, j] = invgamma.rvs(
                        a=setup.ig_a[i][j],
                        scale=setup.ig_b[i][j],
                        size=n_samples,
                    ).reshape(1, n_samples)
    s2_expand = [
        np.empty([n_samples, len(setup.ys[i])]) for i in range(setup.nexp)
    ]
    for i in range(setup.nexp):
        for j in range(len(setup.ig_a[i])):
            s2_expand[i][:, setup.s2_ind[i] == j] = s2[i][:, j].reshape(
                n_samples, 1
            )
    ...
    def neg_log_lik(y):
        ...
        if (
            ...
            disc_y = [
                (setup.models[i].D @ v_cur_list[i]).reshape(1, -1)
                if setup.models[i].nd > 0
                else np.repeat(0, len(setup.ys[i])).reshape(1, -1)
                for i in range(setup.nexp)
            ]
            pred_y = [
                setup.models[i].eval(
                    sc.tran_unif(
                        theta_cur, setup.bounds_mat, setup.bounds.keys()
                    ),
                    pool=True,
                )
                for i in range(setup.nexp)
            ...
            loglik_y = [
                ...
                * (
                    (
                        (
                            ((setup.ys[i] - pred_y[i] - disc_y[i]) ** 2)
                            / s2_expand[i]
                        )
                        + np.log(s2_expand[i])
                    ).sum(axis=1)
            ...
            loglik_y_disc = [
                -0.5
                * (
                    ((v_cur_list[i] ** 2) / setup.models[i].discrep_tau)
                    + np.log(setup.models[i].discrep_tau)
                ).sum()
                if setup.models[i].nd > 0
                else 0
                for i in range(setup.nexp)
            ]
            llik = np.nansum(loglik_y, axis=0) + np.nansum(
                np.hstack(loglik_y_disc)
            )
            llik_max = llik.max()
            llik = llik - llik_max
            ...
            if str(type(dist_func)) != "<class 'NoneType'>":
                VAL = setup.distConstraints(
                    sc.tran_unif(y, setup.bounds_mat, setup.bounds.keys()),
                    setup.bounds,
                )
    ...
    if str(type(disc_init)) != "<class 'NoneType'>":
        param_cur = np.append(
            sc.normalize(theta_init.flatten(), setup.bounds_mat),
            np.hstack(disc_init),
        )
    else:
    ...
    if optmethod == "bh":
        ...
        res = pyswarm.pso(
            neg_log_lik,
            lb=np.repeat(0, len(param_cur.flatten())),
            ub=np.repeat(1, len(param_cur.flatten())),
            ieqcons=[my_constraint],
            swarmsize=niter,
            debug=True,
            omega=0.9,
            maxiter=niter,
        ...
        sample = np.append(
            sc.normalize(theta_init.flatten(), setup.bounds_mat).reshape(1, -1),
            sample,
            axis=0,
        )
        CONSTRAINTS = setup.checkConstraints(
        ...
        if str(type(disc_init)) != "<class 'NoneType'>":
            ...
            def run_mcmc_in_parallel(i):
                disc_rand = np.hstack(disc_init) + np.random.normal(
                    loc=np.repeat(0, len(disc_init.flatten())),
                    scale=np.sqrt(
                        np.repeat(
                            setup.models[0].discrep_tau,
                            len(disc_init.flatten()),
                        )
                    ),
                    size=len(disc_init.flatten()),
                )
                res = minimize(
                    neg_log_lik,
                    np.append(
                        sample[i, :].reshape(1, -1),
                        disc_rand.reshape(1, -1),
                        axis=1,
                    ).flatten(),
                    method="COBYLA",
                    constraints=non_linear_constr,
    ...
    res_trans = sc.tran_unif(
        res_trans0[0 : setup.p], setup.bounds_mat, setup.bounds.keys()
    )
...
def eval_partialintlogposterior_impalapool(
    ...
    if str(type(disc_v)) != "<class 'NoneType'>":
        loglik = -1 * np.apply_along_axis(
            neg_log_lik,
            1,
            np.append(sc.normalize(theta, setup.bounds_mat), disc_v, axis=1),
        )
    else:
        loglik = -1 * np.apply_along_axis(
            neg_log_lik, 1, sc.normalize(theta, setup.bounds_mat)
        )
    return loglik
...
def eval_fullintlogposterior_impalapool(setup, n_samples=1000, theta=None):
    ...
    for i in range(setup.nexp):
        ...
        if i == 0:
            v_cur[:, 0 : disc_dims[i]] = np.random.normal(
                size=n_samples * disc_dims[i],
                loc=0,
                scale=np.sqrt(setup.models[i].discrep_tau),
            ).reshape(n_samples, disc_dims[i])
        else:
            v_cur[
                :, np.sum(disc_dims[0:i]) : np.sum(disc_dims[0 : (i + 1)])
            ] = np.random.normal(
                size=n_samples * disc_dims[i],
                loc=0,
                scale=np.sqrt(setup.models[i].discrep_tau),
    ...
    v_cur_list = [
        ...
        if i == 0
            ...
            :, int(np.sum(disc_dims[0 : (i - 1)])) : int(np.sum(disc_dims[0:i]))
        ]
        for i in range(setup.nexp)
    ]
    ...
    def neg_log_lik(y):
        ...
        disc_y = [
            ...
            if setup.models[i].nd > 0
            else np.repeat(0, len(setup.ys[i])).reshape(n_samples, -1)
            for i in range(setup.nexp)
        ]
        loglik_y = [np.empty([n_samples, 1]) for i in range(setup.nexp)]
        for i in range(setup.nexp):
